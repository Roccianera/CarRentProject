...
public class OptionalAutoDAO {
	...
	public int create() {
		...
		try {
			...
			("insert into OptionalAuto(Id,Descrizione,PrezzoPrefissato)"+" values(?,?,?)");
	...
	public boolean read(int id) {
		...
		try {
			...
			("select * from OptionalAuto where Id= '"+id+"'");
			
			if(res.next()){//se c'è un record da leggere
				this.id=res.getInt("Id");
				descrizione=res.getString("Descrizione");
				prezzoPrefissato=res.getFloat("PrezzoPrefissato");
				presente=true;//l'optional auto con quell'id è presente
			}
